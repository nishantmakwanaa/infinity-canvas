    ...
    <div className="fixed top-16 right-4 z-50 w-52 border border-border bg-card p-3 space-y-3 animate-fade-in">
      ...
      {activeTool === 'shape' && (
        <div>
          ...
          <div className="flex gap-1 mt-1.5">
            {(['rectangle', 'ellipse'] as const).map((s) => (
              <button
                key={s}
                className={`flex-1 h-7 text-[10px] font-mono border transition-colors ${toolSettings.shapeType === s ? 'bg-foreground text-background border-foreground' : 'bg-card text-foreground border-border hover:bg-accent'}`}
                onClick={() => setToolSettings({ shapeType: s })}
              >
                {s === 'rectangle' ? 'Rect' : 'Ellipse'}
              </button>
            ))}
          </div>
        </div>
